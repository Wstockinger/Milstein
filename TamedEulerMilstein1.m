...
function [X,X3,X8,X9] = TamedEulerMilstein1(M,N,T,L)
  ...
  X9(1,:)=X0; 
   ...
   for m=1:M1
      ...
      for n=1:N
          ...
          %X(m+1,n)= X(m,n) + b*h./(1+ M^(-1)*abs(b)) +X(m,n)*mm*dW(N+n,m) + 0.5*(X(m,n)*mm^2)*(dW(N+n,m)^2-h);
          X3(m+1,n)= X3(m,n) + b*h./(1+ M1^(-1)*abs(b)) + mm*dW(n,m);
          %X(m+1,n)= X(m,n) + b*h./(1+ M^(-1)*abs(b)) +mm*dW(N+n,m) ;
      end
   end
   ...
   for m=1:M1
      ...
      for n=1:N
          ...
          for l=1:N
           ...
           levy = levy + mm*(0.5*dW(n,m)*dW(l,m) -0.5*tt + levytemp);
          end  
         
          X9(m+1,n)= X9(m,n) + b*h./(1+ M1^(-1)*abs(b)) + mm*dW(n,m) + levy./N;
          %X(m+1,n)= X(m,n) + b*h./(1+ M^(-1)*abs(b)) +mm*dW(N+n,m) ;
          temp = temp +1;
      end
    end
